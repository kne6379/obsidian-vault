...
- 핵심 정보가 누락되지 않되, 불필요하게 장황한 설명은 피한다.
- 긴 설명 시 문단 구분 또는 글머리 기호를 활용한다.
- 한 섹션이 지나치게 길면 하위 섹션으로 분리한다.
- 추측/모호 표현을 피한다. "~일 수 있습니다", "~인 것 같습니다"를 최소화한다.

## 품질 체크리스트

- 프론트매터가 올바른 형식인지 확인한다.
- 문서 유형에 맞는 섹션 구조를 따른다.
- 비전문가도 이해할 수 있는 수준으로 설명한다.
- 실무 관점의 장단점/고려사항을 포함한다.
- 관련 문서 링크를 적절히 연결한다.
...
- 새 문서 작성 시 `develop/_templates/CONCEPT.md` 템플릿을 기반으로 한다.
- 새 문서 작성 전 반드시 `develop/_meta/INDEX.md`를 읽어 중복을 방지한다.
- 새 용어 작성 시 `develop/_meta/GLOSSARY.md`를 확인한다.
