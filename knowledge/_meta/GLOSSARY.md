...
이 문서는 `knowledge` 폴더 내 문서에서 사용되는 주요 용어를 정의합니다. 새로운 용어가 등장할 때마다 이 문서를 업데이트하여 일관된 용어 사용을 유지합니다.
...
**말하기** (Telling) - 글쓰기에서 작가가 감정·상태·판단을 직접 서술하는 방식. "슬펐다", "좋은 시간이었다"처럼 결론을 독자에게 전달하는 서술 기법
...
**보여주기** (Showing) - 글쓰기에서 감정이나 상황을 직접 설명하지 않고, 구체적인 장면·감각·행동을 통해 독자가 스스로 느끼도록 유도하는 서술 기법. "Show, don't tell" 원칙의 핵심
...
**테스트 더블** (Test Double) - 프로덕션 객체를 대체하는 테스트용 객체의 총칭. 더미, 스텁, 페이크, 스파이, 목의 다섯 가지 유형이 있음

**테스트 피라미드** (Test Pyramid) - 단위 테스트(다수) → 통합 테스트(중간) → E2E 테스트(소수)로 구성되는 테스트 전략 모델. Mike Cohn이 제안
...
**ngrok** - 로컬 머신에서 실행 중인 서비스를 공개 URL을 통해 인터넷에 노출하는 리버스 프록시 터널링 도구. 아웃바운드 TLS 연결로 방화벽 변경 없이 동작

**TDD** (Test-Driven Development) - 실패하는 테스트를 먼저 작성하고, 통과하는 최소 코드를 구현한 뒤, 리팩터링하는 개발 방법론. Kent Beck이 정립
